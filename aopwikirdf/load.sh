...
-- Clean up graphs and previous load state
SPARQL CLEAR GRAPH <http://aopwiki.org/graph/>;
...
DB.DBA.VHOST_REMOVE(lpath=>'/sparql', vhost=>'*ini*', lhost=>'*ini*');
DB.DBA.VHOST_DEFINE(lpath=>'/sparql', ppath=>'/!sparql/', is_dav=>1, vsp_user=>'dba', opts=>vector('noinherit', 1, 'cors', '*', 'cors_restricted', 0));
