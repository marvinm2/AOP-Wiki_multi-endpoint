...
build_load_funcs() {
    ...
    # Versioning catalogue + SPARQL service description (no date in the name) go
    # into a dedicated metadata graph so they're queryable and the endpoint can
    # advertise its versions. Generated by generate_catalog.py.
    local meta_graph="http://aopwiki-multirdf.vhp4safety.nl/metadata"
    for metafile in "ServiceDescription.ttl" "AOPWikiRDF-Catalog.ttl"; do
        if [[ -f "${SCRIPT_DIR}/aopwikirdf/${metafile}" ]]; then
            echo "ld_dir('${container_data_dir}', '${metafile}', '${meta_graph}');"
        fi
    done
}
